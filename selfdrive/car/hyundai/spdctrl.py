...
class Spdctrl(SpdController):
    ...
    def update_lead(self, sm, CS, dRel, yRel, vRel):
        ...
        # 선행차량이 멀리 있는 상태에서 감속 조건
        elif 30 < dRel < 149 and lead_objspd < -15: #정지 차량 및 급감속 차량 발견 시
            if dRel > 100:
                self.seq_step_debug = "d>100감속"
                lead_wait_cmd, lead_set_speed = self.get_tm_speed(CS, 120+lead_objspd, -1)
            elif dRel > 80:
                self.seq_step_debug = "d>80감속"
                lead_wait_cmd, lead_set_speed = self.get_tm_speed(CS, 100+lead_objspd, -1)
            elif dRel > 60:
                self.seq_step_debug = "d>60감속"
                lead_wait_cmd, lead_set_speed = self.get_tm_speed(CS, 80+lead_objspd, -1)
            elif dRel > 40:
                self.seq_step_debug = "d>40감속"
                lead_wait_cmd, lead_set_speed = self.get_tm_speed(CS, 60+lead_objspd, -1)
            elif dRel > 20:
                self.seq_step_debug = "d>20감속"
                lead_wait_cmd, lead_set_speed = self.get_tm_speed(CS, 40+lead_objspd, -1)
            ...
            lead_wait_cmd, lead_set_speed = self.get_tm_speed( CS, 60, 4)
